...
fn solve() {
    let nums1: HashSet<_> = rin_vec!(i32).into_iter().collect();
    let nums2: HashSet<_> = rin_vec!(i32).into_iter().collect();

    let ans: BTreeSet<_> = nums1.intersection(&nums2).cloned().collect();
    ans.iter().for_each(|i| rout!(i));
